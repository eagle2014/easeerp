...
var thisPage = {
        ...
        this.toolbar=new TS.TB("tbContainer",[
		    "|",
		    {
			    ...
			    handler:function(button){
		            ...
		            TS.openWindow({
		                url: TS.rootPath +"unitAction.do?action=Create", 
		                title: "新建单位",
		                id: "CreateUnit"
		            });
			    },
	...
	initGrid: function(){
        ...
        dataGrid = new TS.Grid("gridContainer",{
	 	    data: TS.rootPath + "unitAction.do?action=View",
	 	    ...
	 	    cm:[
	 		    ...
	 		    {id: "FullCode",text: "编码",width: 200 },
	 		    {id: "LevelName",text: "级别", width: 100 },
		    ...
		    dblClickRowHandler:function(id,rowData,row){
                ...
                TS.openWindow({
                    url: TS.rootPath +"unitAction.do?action=Open&id=" + id, 
                    title: "单位：" + row.Name,
                    id: row.Unid,
                    tabTip: row.UnitFullName + "[" + row.UnitFullCode + "]"
                });
		    }
        });
	...
	doDelete: function(){
        ...
        if(ids.length == 0){
            TS.Msg.msgBox({title:"单位配置",msg:"请先选择要删除的单位信息！"});
            return;
        }
        
        TS.Msg.confirm({title:"单位配置",msg:"确定要删除选中的单位信息吗？",onYes: function(){
            // 通过Ajax执行删除操作    
            ...
            { 
                ...
                onSuccess: function(transport){
                    ...
                    if (response.result == true){
                        refresh();
                        TS.Msg.hide();
    				    //TS.Msg.msgBox({title:'单位配置',msg:response.msg});
                    }else{
    			        TS.Msg.msgBox({title:"单位配置", msg:response.msg, iconCls: TS.Msg.IconCls.ERROR});
                    }
                },
                onFailure: function(transport){
 				    TS.Msg.msgBox({title:"单位配置",msg: '删除单位过程出现异常!', iconCls: TS.Msg.IconCls.ERROR});
                }
            });	
        }});
    }
...
function renderHref(value,rowData,rowJson){
	...
	_href += "TS.openWindow({url:\"" + TS.rootPath + "unitAction.do?action=Open&id=" + rowJson.ID + "\",";
	_href += "title:\"单位：" + rowJson.Name +  "\",";
