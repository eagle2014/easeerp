...
Egd.ATM = {
    ...
    uploadFile: function(config){
		Egd.applyIf(config,{
            ...
            afterOk: function(retult){
                Egd.ATM.updateCaller(retult);   // 更新附件列表
            }
		});
		var url = Egd.rootPath + 'Egd_Attachment/UploadFile.aspx';
    ...
    downloadFile: function(config){
        var unid = Egd.ATM.getSelected(config.caller);
        if(unid){
		    var url = Egd.rootPath + 'Egd_Attachment/DownloadFile.aspx?type=unid&id=' + unid;
            window.open(url,'blank');
        }else{
            Egd.Msg.msgBox({msg: '请先选择要下载的附件！'});
        }
    },
    ...
    deleteFile: function(config){
        var unid = Egd.ATM.getSelected(config.caller);
        if(unid){
		    var url = Egd.rootPath + 'attachmentAction.do?action=Delete&type=unid&ids=' + unid;
		    Egd.Ajax.request({
		        url: url,
    ...
    getSelected: function(caller){
	    var selectedRadio = Egd.getSelectedRadio(caller + '_SELECT');
	    if(selectedRadio){
