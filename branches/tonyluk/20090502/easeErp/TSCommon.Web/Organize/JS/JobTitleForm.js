var thisPage = {
	...
	doSave:function(){
	    ...
	    var strUrl = TS.rootPath + "jobTitleAction.do?action=SaveByAjax";
		...
		{ 
			...
			onSuccess: function(transport){
				...
				if (response.result == true){
    				...
    				TS.Msg.msgBox({title:"职务配置",msg: response.msg, iconCls: TS.Msg.IconCls.INFO});// 显示保存成功的提示信息
				}else{
    				TS.Msg.msgBox({title:"职务配置",msg: response.msg, iconCls: TS.Msg.IconCls.ERROR});
				}
			},
			onFailure: function(transport){
				TS.Msg.msgBox({title:"职务配置",msg: '保存职务过程出现异常!', iconCls: TS.Msg.IconCls.ERROR});
			}
		});	
    },
	...
	doEdit:function(){
        ...
        var strUrl = TS.rootPath + "jobTitleAction.do?action=Edit";
        strUrl += "&id=" + $F("_ID");
        strUrl += "&timeStamp=" + new Date().getTime();
	...
	doDelete:function(){
	    // 删除确定
        TS.Msg.confirm({title:"职务配置",msg:"确定要删除当前职务吗？",onYes: function(){
