var thisPage = {
	...
	validate:function(){
		...
		var fieldDescs = "模块名称;模块代码;模块顺序号";
	...
	doSave:function(){
	    ...
	    var strUrl = TS.rootPath + "modelAction.do?action=SaveByAjax";
		...
		{ 
			...
			onSuccess: function(transport){
				...
				if (response.result == true){
    				...
    				TS.Msg.msgBox({title:"模块配置",msg: response.msg, iconCls: TS.Msg.IconCls.INFO});// 显示保存成功的提示信息
				}else{
    				TS.Msg.msgBox({title:"模块配置",msg: response.msg, iconCls: TS.Msg.IconCls.ERROR});
				}
			},
			onFailure: function(transport){
				TS.Msg.msgBox({title:"模块配置",msg: '保存模块过程出现异常!', iconCls: TS.Msg.IconCls.ERROR});
			}
		});	
    },
	...
	doEdit:function(){
        ...
        var strUrl = TS.rootPath + "modelAction.do?action=Edit";
        strUrl += "&id=" + $F("_ID");
        strUrl += "&timeStamp=" + new Date().getTime();
	...
	doDelete:function(){
	    // 删除确定
        TS.Msg.confirm({title:"模块配置",msg:"确定要删除当前模块吗？",onYes: function(){
...
function onType_Changed(){
    ...
    if(selectedValue == "SubModel"){
        $('TR_MODEL_ID').style.display="";
    }else{
        $('TR_MODEL_ID').style.display="none";
