...
function DlgOkFn(){
    ...
    if(optionsObj.selectedIndex == -1){
        TS.Msg.msgBox({title:"选择用户", msg:"请先选择用户！", iconCls: TS.Msg.IconCls.WARNING});
	    return null;
    }
    ...
    if(my.singleSelect){
        ...
        for(var i = 0; i < _options.length; i++){
            if(_options[i].selected){
                selectedDomains.push(domains[i]);
            }
        }
...
var thisPage = {
    ...
    clickTreeNode: function(node, e){
	    ...
	    var strUrl = TS.rootPath + "userInfoAction.do?action=FindUserInfoByOU&ouUnid=" + node.id;
	    ...
	    new Ajax.Request(strUrl, {	
	        method: 'get', 
	  	    onSuccess: function(transport){
			    ...
			    for(var i = 0; i < domains.length; i++){
				    selOptionObj.options[selOptionObj.length] = new Option(domains[i].Name, domains[i].Unid);
			    }
		    },
