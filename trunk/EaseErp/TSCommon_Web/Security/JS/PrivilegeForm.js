...
var thisPage = {
	...
	doSave:function(){
	    ...
	    var strUrl = TS.rootPath + "privilegeAction.do?action=SaveByAjax";
		...
		{ 
			...
			onSuccess: function(transport){
				...
				if (response.result == true){
    				...
    				TS.Msg.msgBox({title:"权限配置",msg: response.msg, iconCls: TS.Msg.IconCls.INFO});// 显示保存成功的提示信息
				}else{
    				TS.Msg.msgBox({title:"权限配置",msg: response.msg, iconCls: TS.Msg.IconCls.ERROR});
				}
			},
			onFailure: function(transport){
				TS.Msg.msgBox({title:"权限配置",msg: '保存权限过程出现异常!', iconCls: TS.Msg.IconCls.ERROR});
			}
		});	
    },
	...
	doEdit:function(){
        ...
        var strUrl = TS.rootPath + "privilegeAction.do?action=Edit";
        strUrl += "&id=" + $F("_ID");
        strUrl += "&timeStamp=" + new Date().getTime();
	...
	doDelete:function(){
	    // 删除确定
        TS.Msg.confirm({title:"权限配置",msg:"确定要删除当前权限吗？",onYes: function(){
			    ...
			    onSuccess: function(transport){
				    var response = eval("("+transport.responseText+")");  
 				    if (response.result == true){
    			        ...
    			        TS.Msg.msgBox({title:"权限", msg:response.msg, iconCls: TS.Msg.IconCls.ERROR});
				    }
			    },
			    onFailure: function(transport){
 				    TS.Msg.msgBox({title:"权限",msg: '删除权限过程出现异常!', iconCls: TS.Msg.IconCls.ERROR});
			    }
		    });	
        }});
    },
...
function onType_Changed(){
    var selectedValue = TS.getRadioValue("Type");
    if(selectedValue == "url")
        $('TR_URL_PATH').style.display="";
    else
        $('TR_URL_PATH').style.display="none";
}
